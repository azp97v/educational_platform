...
const ThemeManager = {
    ...
    initTheme() {
        ...
        AppSystem.log(`🎨 تم تحميل المظهر: ${savedTheme}`);
    },
    ...
    toggle() {
        ...
        AppSystem.log(`🌓 تم تبديل المظهر إلى: ${newTheme}`);
    ...
    setTheme(theme) {
        if (!['light', 'dark'].includes(theme)) {
            AppSystem.log(`❌ مظهر غير صحيح: ${theme}`, 'error');
            return;
        }
        document.documentElement.setAttribute('data-theme', theme);
        localStorage.setItem('app-theme', theme);
        AppSystem.theme = theme;
        AppSystem.log(`🎨 تم تعيين المظهر: ${theme}`);
    },
...
const NotificationSystem = {
    ...
    show(message, type = 'info', duration = 4000) {
        ...
        notification.innerHTML = `
            <span style="color: ${colors[type] || '#007AFF'}; font-size: 16px;">${icon}</span>
            <span>${message}</span>
            <button style="
                background: none;
                border: none;
